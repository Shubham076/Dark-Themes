...
class CheckLicense {
    ...
    companion object {
         ...
         * Null return value means the LicensingFacade object is not initialized yet => one cannot say for sure whether a valid license for the plugin exists or not.
         * The interpretation of the null value is up to the plugin.
         */
        fun isLicensed(): Boolean? {
            ...
            return when {
                cstamp.startsWith(KEY_PREFIX) -> isKeyValid(cstamp.removePrefix(KEY_PREFIX))
                cstamp.startsWith(STAMP_PREFIX) -> isLicenseServerStampValid(cstamp.removePrefix(STAMP_PREFIX))
                else -> false
            }
        }

        fun requestLicense(message: String) {
            // Ensure the dialog is displayed from the UI thread and in a non-modal context
        ...
        private fun showRegisterDialog(productCode: String, message: String) {
            ...
            var registerAction = actionManager.getAction("RegisterPlugins") // Assume running in open source version

            if (registerAction == null) {
                registerAction = actionManager.getAction("Register") // Assume commercial IDE
            }

            registerAction?.let {
                ActionUtil.performActionDumbAwareWithCallbacks(
                    it,
                    AnActionEvent(null, asDataContext(productCode, message), "", Presentation(), ActionManager.getInstance(), 0)
                )
            }
        }
        ...
        private fun isKeyValid(key: String): Boolean {
            ...
            return try {
                ...
                sig.initVerify(createCertificate(Base64.getMimeDecoder().decode(certBase64.toByteArray()), emptyList(), false))

                val licenseBytes = Base64.getMimeDecoder().decode(licensePartBase64.toByteArray())
                sig.update(licenseBytes)

                sig.verify(Base64.getMimeDecoder().decode(signatureBase64.toByteArray())) && licenseBytes.toString(Charsets.UTF_8)
        ...
        private fun isLicenseServerStampValid(serverStamp: String): Boolean {
            return try {
                ...
                val signatureBytes = base64.decode(parts[4].toByteArray())
                val certBytes = base64.decode(parts[5].toByteArray())

                val intermediateCerts = parts.drop(6).map { base64.decode(it.toByteArray()) }
                val sig = Signature.getInstance(signatureType)

                sig.initVerify(createCertificate(certBytes, intermediateCerts, true))
                sig.update("$timeStamp:$machineId".toByteArray(Charsets.UTF_8))

                sig.verify(signatureBytes) && expectedMachineId == machineId && Math.abs(
        ...
        private fun createCertificate(certBytes: ByteArray, intermediateCertsBytes: List<ByteArray>, checkValidityAtCurrentDate: Boolean): X509Certificate {
            ...
            val allCerts = HashSet<Certificate>().apply {
                add(cert)
                intermediateCertsBytes.forEach { add(x509Factory.generateCertificate(ByteArrayInputStream(it))) }
            }

            return try {
                val selector = X509CertSelector().apply { certificate = cert }
                val trustAnchors = ROOT_CERTIFICATES.map {
                    TrustAnchor(x509Factory.generateCertificate(ByteArrayInputStream(it.toByteArray())) as X509Certificate, null)
                }.toSet()

                val pkixParams = PKIXBuilderParameters(trustAnchors, selector).apply {
                    isRevocationEnabled = false
                    if (!checkValidityAtCurrentDate) date = cert.notBefore
                ...
                throw Exception("Certificate used to sign the license is not signed by JetBrains root certificate", e)
            }
        }
    }
}